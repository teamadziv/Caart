...
async function authCallback({
  ...
  try {
    ...
    if (config.useOnlineTokens && !callbackResponse.session.isOnline) {
      await config.logger.debug('Completing offline token OAuth, redirecting to online token OAuth', {
        shop: callbackResponse.session.shop
      });
...
async function registerWebhooks(config, api, session) {
  await config.logger.debug('Registering webhooks', {
    shop: session.shop
  });
  ...
  for (const topic in responsesByTopic) {
    ...
    for (const response of responsesByTopic[topic]) {
      if (!response.success && !shopifyApi.gdprTopics.includes(topic)) {
        ...
        if (result.errors) {
          await config.logger.error(`Failed to register ${topic} webhook: ${result.errors[0].message}`, {
            shop: session.shop
          });
        } else {
          await config.logger.error(`Failed to register ${topic} webhook: ${JSON.stringify(result.data)}`, {
            shop: session.shop
          });
        }
      }
