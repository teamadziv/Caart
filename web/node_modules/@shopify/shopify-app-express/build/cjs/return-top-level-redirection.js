...
async function returnTopLevelRedirection({
  res,
  config,
  bearerPresent,
  redirectUrl
  ...
  // re-authenticate
  if (bearerPresent) {
    await config.logger.debug(`Redirecting to top level at ${redirectUrl} while embedded, returning headers`);
    res.status(403);
    res.header('X-Shopify-API-Request-Failure-Reauthorize', '1');
    res.header('X-Shopify-API-Request-Failure-Reauthorize-Url', redirectUrl);
    res.end();
  } else {
    await config.logger.debug(`Redirecting to ${redirectUrl} while at the top level`);
