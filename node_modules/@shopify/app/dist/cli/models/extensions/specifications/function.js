...
const spec = createExtensionSpecification({
    ...
    deployConfig: async (config, directory, apiKey, moduleId) => {
        ...
        const targets = config.targeting &&
            (await Promise.all(config.targeting.map(async (config) => {
                let inputQuery;
    ...
    preDeployValidation: async (extension) => {
        ...
        if (!wasmExists) {
            throw new AbortError(outputContent `The function extension "${extension.handle}" hasn't compiled the wasm in the expected path: ${extension.outputPath}`, `Make sure the build command outputs the wasm in the expected directory.`);
        }
    },
});
async function readInputQuery(path) {
    if (await fileExists(path)) {
        return readFile(path);
    }
    else {
        throw new AbortError(`No input query file at ${path}.`, `Create the file or remove the line referencing it in the extension's TOML.`);
    }
}
export default spec;
