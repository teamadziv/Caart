import { WebConfigurationSchema, App, WebType, isCurrentAppSchema, getAppScopesArray, AppSchema, LegacyAppSchema, } from './app.js';
...
const noopAbortOrReport = (errorMessage, fallback, configurationPath) => fallback;
export async function loadConfigurationFile(filepath, abortOrReport = (errorMessage) => {
    throw new AbortError(errorMessage);
}, decode = decodeToml) {
    if (!(await fileExists(filepath))) {
        return abortOrReport(outputContent `Couldn't find the configuration file at ${outputToken.path(filepath)}`, '', filepath);
    }
    try {
        ...
        if (err.line && err.pos && err.col) {
            return abortOrReport(outputContent `Fix the following error in ${outputToken.path(filepath)}:\n${err.message}`, null, filepath);
        }
        else {
            throw err;
        }
...
const isCurrentSchema = (schema) => {
    const currentSchemaOptions = AppSchema.keyof().Values;
    const legacySchemaOptions = LegacyAppSchema.keyof().Values;
...
export async function parseConfigurationFile(schema, filepath, abortOrReport = (errorMessage) => {
    throw new AbortError(errorMessage);
}, decode = decodeToml) {
...
export async function parseConfigurationObject(schema, filepath, configurationObject, abortOrReport) {
    const fallbackOutput = {};
    const parseResult = schema.safeParse(configurationObject);
    if (!parseResult.success) {
        const formattedError = JSON.stringify(parseResult.error.issues, null, 2);
        return abortOrReport(outputContent `Fix a schema error in ${outputToken.path(filepath)}:\n${formattedError}`, fallbackOutput, filepath);
    }
    return { ...parseResult.data, path: filepath };
}
export function findSpecificationForType(specifications, type) {
    return specifications.find((spec) => spec.identifier === type || spec.externalIdentifier === type || spec.additionalIdentifiers?.includes(type));
}
...
export function getDotEnvFileName(configurationPath) {
    const configurationShorthand = getAppConfigurationShorthand(configurationPath);
    return configurationShorthand ? `${dotEnvFileNames.production}.${configurationShorthand}` : dotEnvFileNames.production;
}
export async function loadDotEnv(appDirectory, configurationPath) {
    let dotEnvFile;
...
class AppLoader {
    constructor({ directory, configName, mode, specifications }) {
    ...
    parseConfigurationFile(schema, filepath, 
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    decode = decodeToml) {
    ...
    async loaded() {
        ...
        const { allExtensions, usedCustomLayout } = await this.loadExtensions(appDirectory, configuration.extension_directories);
        ...
        const { webs, usedCustomLayout: usedCustomLayoutForWeb } = await this.loadWebs(appDirectory, configuration.web_directories);
        const usesWorkspaces = await appUsesWorkspaces(appDirectory);
        const appClass = new App(name, 'SHOPIFY_API_KEY', appDirectory, packageManager, configuration, nodeDependencies, webs, allExtensions, usesWorkspaces, dotenv);
    ...
    async loadWebs(appDirectory, webDirectories) {
    ...
    validateWebs(webs) {
        ...
        [WebType.Backend, WebType.Frontend].forEach((webType) => {
            ...
            if (websOfType.length > 1) {
                this.abortOrReport(outputContent `You can only have one web with the ${outputToken.yellow(webType)} role in your app`, undefined, joinPath(websOfType[1].directory, configurationFileNames.web));
            }
        });
    }
    ...
    async createExtensionInstance(type, configurationObject, configurationPath, directory) {
        const specification = findSpecificationForType(this.specifications, type);
        if (!specification) {
            return this.abortOrReport(outputContent `Invalid extension type "${type}" in "${relativizePath(configurationPath)}"`, undefined, configurationPath);
        }
        const configuration = await parseConfigurationObject(specification.schema, configurationPath, configurationObject, this.abortOrReport.bind(this));
    ...
    async loadExtensions(appDirectory, extensionDirectories) {
            ...
            if (extensions) {
                ...
                const extensionsInstancesPromises = configuration.extensions.map(async (extensionConfig) => {
                    ...
                    if (!restConfig.handle) {
                        ...
                        this.abortOrReport(outputContent `Missing handle for extension "${restConfig.name}" at ${relativePath(appDirectory, configurationPath)}`, undefined, configurationPath);
                ...
                return this.abortOrReport(outputContent `Invalid extension type at "${outputToken.path(relativePath(appDirectory, configurationPath))}". Please specify a type.`, undefined, configurationPath);
            }
        });
        ...
        allExtensions.forEach((extension) => {
            if (extension.handle && handles.has(extension.handle)) {
                ...
                this.abortOrReport(outputContent `Duplicated handle "${handle}" in extensions ${result}. Handle needs to be unique per extension.`, undefined, extension.configuration.path);
            }
    ...
    async findEntryPath(directory, specification) {
        let entryPath;
        if (specification.appModuleFeatures().includes('single_js_entry_path')) {
            ...
            if (!entryPath) {
                this.abortOrReport(outputContent `Couldn't find an index.{js,jsx,ts,tsx} file in the directories ${outputToken.path(directory)} or ${outputToken.path(joinPath(directory, 'src'))}`, undefined, directory);
            }
        }
        else if (specification.identifier === 'function') {
            entryPath = (await Promise.all(['src/index.js', 'src/index.ts', 'src/main.rs']
                .map((relativePath) => joinPath(directory, relativePath))
                .map(async (sourcePath) => ((await fileExists(sourcePath)) ? sourcePath : undefined)))).find((sourcePath) => sourcePath !== undefined);
        }
        return entryPath;
 ...
 */
export async function loadAppConfiguration(options) {
...
class AppConfigurationLoader {
    constructor({ directory, configName }) {
        this.directory = directory;
        this.configName = configName;
    }
    async loaded() {
        const appDirectory = await this.getAppDirectory();
        let configSource = this.configName ? 'flag' : 'cached';
        ...
        let configurationLoadResultMetadata = {
    ...
    async getAppDirectory() {
        ...
        const appDirectory = await findPathUp(async (directory) => {
        ...
        if (appDirectory) {
            ...
            throw new AbortError(outputContent `Couldn't find the configuration file for ${outputToken.path(this.directory)}, are you in an app directory?`);
        }
    }
    async getConfigurationPath(appDirectory) {
        ...
        const entries = (await Promise.all(candidates.map(async (candidateFile) => {
            try {
                ...
                if (configuration.client_id !== undefined) {
                    ...
                    return [basename(candidateFile), configuration.client_id];
                }
                // eslint-disable-next-line no-catch-all/no-catch-all
            }
            catch {
...
export function isWebType(web, type) {
...
async function logMetadataForLoadedApp(app, loadingStrategy) {
    await metadata.addPublicMetadata(async () => {
        ...
        const webBackendFramework = webBackendCount === 1 ? app.webs.filter((web) => isWebType(web, WebType.Backend))[0]?.framework : undefined;
...
async function logMetadataFromAppLoadingProcess(loadMetadata) {
    await metadata.addPublicMetadata(async () => {
        return {
...
export function getAppConfigurationFileName(configName) {
    if (!configName) {
...
export function getAppConfigurationShorthand(path) {
