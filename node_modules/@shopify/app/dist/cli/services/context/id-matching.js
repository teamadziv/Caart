...
function migrateLegacyFunctions(ids, localSources, remoteSources) {
    const migrated = {};
    ...
    remoteSources
        ...
        if (legacyId)
            pendingMigrations[legacyId] = functionExtension.uuid;
        const legacyUuid = parsedConfig.legacy_function_uuid;
        if (legacyUuid)
            pendingMigrations[legacyUuid] = functionExtension.uuid;
    });
    localSources
        .filter((extension) => extension.type === 'function')
        .forEach((functionExtension) => {
...
export async function automaticMatchmaking(localSources, remoteSources, identifiers, remoteIdField) {
    ...
    const { migrated: migratedFunctions, pending: pendingAfterMigratingFunctions } = migrateLegacyFunctions(ids, localSources.filter((local) => !existsRemotely(local)), remoteSources.filter((remote) => !localUUIDs.includes(remote[remoteIdField])));
    ...
    const { matched: matchedByNameAndType, pending: matchResult } = matchByNameAndType(pendingAfterMigratingFunctions.local, pendingAfterMigratingFunctions.remote, remoteIdField);
