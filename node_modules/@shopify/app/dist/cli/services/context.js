...
import { fetchAllDevStores, fetchAppDetailsFromApiKey, fetchOrgAndApps, fetchOrganizations, fetchOrgFromId, fetchStoreByDomain, fetchAppExtensionRegistrations, } from './dev/fetch.js';
...
import { renderError, renderInfo, renderTasks } from '@shopify/cli-kit/node/ui';
...
const appFromId = async (appId, token) => {
    ...
    if (!app)
        throw new BugError([`Couldn't find the app with Client ID`, { command: appId }], resetHelpMessage);
    return app;
};
...
export async function fetchOrCreateOrganizationApp(app, token, directory) {
...
export async function fetchAppAndIdentifiers(options, token, reuseFromDev = true) {
 ...
 */
export async function getAppContext({ reset, directory, token, configName, commandConfig, promptLinkingApp = true, }) {
...
function showReusedDevValues({ organization, selectedApp, selectedStore, cachedInfo }) {
    if (!cachedInfo)
        return;
    const usingDifferentSettings = selectedApp.apiKey !== cachedInfo?.appId || selectedStore.shopDomain !== cachedInfo.storeFqdn;
...
export function renderCurrentlyUsedConfigInfo({ org, appName, devStore, updateURLs, configFile, appDotEnv, resetMessage, }) {
...
export function showReusedGenerateValues(org, cachedAppInfo) {
    renderCurrentlyUsedConfigInfo({
        org,
...
export function showReusedDeployValues(org, app, remoteApp) {
    renderCurrentlyUsedConfigInfo({
        org,
...
export async function enableDeveloperPreview({ apiKey, token }) {
    return developerPreviewUpdate({ apiKey, token, enabled: true });
}
export async function disableDeveloperPreview({ apiKey, token }) {
    await developerPreviewUpdate({ apiKey, token, enabled: false });
}
export async function developerPreviewUpdate({ apiKey, token, enabled, }) {
    let result;
    try {
...
async function logMetadataForLoadedReleaseContext(env, partnerId) {
    await metadata.addPublicMetadata(() => ({
        partner_id: tryParseInt(partnerId),
        api_key: env.partnersApp.apiKey,
    }));
}
function checkDeploymentsBeta(command, partnersApp) {
    const deploymentMode = partnersApp.betas?.unifiedAppDeployment ? 'unified' : 'legacy';
